...
const HotBetCard: React.FC<HotBetCardProps> = ({ bet, darkMode }) => {
  ...
  return (
    <div className={`flex flex-col min-w-[220px] max-w-[240px] rounded-xl shadow-lg mx-2 p-4 border-2 ${darkMode ? 'bg-yellow-900 border-yellow-400' : 'bg-white border-yellow-200'} relative overflow-hidden`} style={{ boxShadow: darkMode ? '0 2px 16px 0 #FCFF52' : '0 2px 16px 0 #FCFF52' }}>
      ...
      <div className={`font-semibold text-base mb-2 ${darkMode ? 'text-white' : 'text-yellow-900'}`}>{bet.content.length > 60 ? bet.content.slice(0, 57) + '...' : bet.content}</div>
      <div className="flex items-center mb-2">
        {bet.pfpUrl ? (
          <img src={bet.pfpUrl} alt={bet.author} className="h-7 w-7 rounded-full object-cover mr-2" />
        ) : (
          <div className={`h-7 w-7 rounded-full flex items-center justify-center font-bold mr-2 ${darkMode ? 'bg-yellow-200 text-yellow-900' : 'bg-yellow-100 text-yellow-800'}`}>{bet.author.charAt(0).toUpperCase()}</div>
        )}
        <span className={`text-xs font-medium ${darkMode ? 'text-yellow-200' : 'text-yellow-800'}`}>{bet.author}</span>
      </div>
      <div className="flex items-center mb-2">
      ...
      <div className="w-full h-3 bg-purple-100 dark:bg-purple-900 rounded-full overflow-hidden mb-1">
        <div className="h-full bg-gradient-to-r from-pink-500 via-yellow-300 to-orange-400" style={{ width: yayPct + '%' }}></div>
      </div>
