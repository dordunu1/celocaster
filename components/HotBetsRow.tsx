  ...
  return (
    <div
      className={`flex items-center min-w-[220px] max-w-[320px] h-[28px] rounded-full shadow-md mx-1 px-2 border ${
        darkMode ? 'bg-yellow-900 border-yellow-400' : 'bg-white border-yellow-200'
      } text-[11px] font-medium whitespace-nowrap overflow-hidden`}
      style={{ boxShadow: darkMode ? '0 1px 2px 0 #FCFF52' : '0 1px 2px 0 #FCFF52' }}
    >
      ...
      <span className={`mr-2 truncate ${darkMode ? 'text-white' : 'text-yellow-900'}`} style={{ maxWidth: 70 }}>{shortContent}</span>
      {bet.pfpUrl ? (
        <img src={bet.pfpUrl} alt={bet.author} className="h-4 w-4 rounded-full object-cover mr-1" />
      ) : (
        <span className={`h-4 w-4 rounded-full flex items-center justify-center font-bold mr-1 text-[10px] ${darkMode ? 'bg-yellow-200 text-yellow-900' : 'bg-yellow-100 text-yellow-800'}`}>{bet.author.charAt(0).toUpperCase()}</span>
      )}
      <span className={`mr-2 ${darkMode ? 'text-yellow-200' : 'text-yellow-800'}`}>{bet.author}</span>
